...
RUN sed -i "s/O2/Os/g" /etc/portage/make.conf \
    ...
    && PYTHON_TARGETS="python3_11" USE="-multilib -split-usr openmp" ROOT=/mini emerge -v -j$(nproc) sys-libs/glibc sys-kernel/linux-headers coreutils sys-apps/portage dev-vcs/git sys-devel/gcc  wget curl vim grep \
